<template>
  <div
    data-cookie-policy
    class="max-w-full w-full mt-2 mx-auto md:max-w-[70%] xl:max-w-[50%]"
  >
    <div class="rounded-2xl w-full">
      <NuxtImg
        ...
        decoding="async"
        fetchpriority="high"
        class="rounded-2xl w-full sm:h-[22rem] object-center mx-auto"
      />
    </div>

    <div class="cookie-policy-wrap mt-6">
      <ContentDoc />
    </div>
  </div>
</template>
...
.cookie-policy-wrap :deep(h3) {
  @apply mt-7 mb-2 text-[1.1rem] font-semibold;
  word-spacing: 0.2rem;
}

.cookie-policy-wrap :deep(ul) {
  @apply py-3 pl-10;
}
.cookie-policy-wrap :deep(ul li) {
  @apply pb-2 text-[.95rem] text-zinc-300;
  list-style-type: disc;
}

.cookie-policy-wrap :deep(ol) {
  @apply py-2 pl-12;
}
.cookie-policy-wrap :deep(ol li) {
  @apply pl-2 pb-2 text-zinc-200;
  list-style-type: decimal;
}
.cookie-policy-wrap :deep(ol li a) {
