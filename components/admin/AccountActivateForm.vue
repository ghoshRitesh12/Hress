<template>
  <form
    ...
    class="flex items-center gap-4 border-[1px] py-[.65rem] px-4 sm:px-5 rounded-2xl w-full border-zinc-700 md:whitespace-nowrap"
  >
    <label
      class="block"
      for="acc-active"
    ...
    <button
      type="submit"
      class="flex items-center justify-center ml-auto px-4 py-2 rounded-xl bg-zinc-800"
    >
      <Icon
        name="charm:tick"
        class="text-xl align-middle text-accent-200 pointer-events-none"
      />
    </button>
  </form>
</template>
...
onMounted(() => {
  formState.value = {
    submitUrl: `/api/admin/profile/${profile.value.referralId}/activate`,
    refreshUrl: `/api/admin/profile/${profile.value.referralId}/about`,
  };
});
...
const submitAccountActivateForm = async () => {
  try {
    ...
    const { data, error } = await useFetch(formState.value.submitUrl, {
      method: "PUT",
      headers,
